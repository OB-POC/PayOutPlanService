...
let selectCreditCards = (creditAccs, totalAvailableBalance,totalMinDue) => {
    ...
    if(totalAvailableBalance >= totalMinDue)
    {
        return creditAccs.map((bankAccount) => {
            ...
            const {accounts} = bankAccount;
            let balanceDue = (accounts[0].accountType == "M")?accounts[0].minMonthlyPayment:accounts[0].totalBalanceDue;
            let minDue = 0;
...
let selectAllCreditCard = (creditAccs) =>{
    return creditAccs.map((bankAccount) =>{
        const {accounts} = bankAccount;
        accounts[0]["clearableAmount"] = (accounts[0].accountType == "M")?accounts[0].minMonthlyPayment:accounts[0].totalBalanceDue;
        return bankAccount;
    })
}
...
let dataEnricher = (bestPlan,cloneCredit,cloneDebit) => {
    ...
    for(let creditAccNo in bestPlan)
    {
        ...
        let totalOutstandingBalance;
        
        //creditDebitMatchObj["clearedTotalDue"] = ;
        if(!creditAccountDetail.accountType.localeCompare("M"))
        {
            //console.log("interest rate",creditAccountDetail.interestRate);
            creditDebitMatchObj["interestRate"] = creditAccountDetail.interestRate;
            creditDebitMatchObj["minMonthlyPayment"] = creditAccountDetail.minMonthlyPayment;
            totalOutstandingBalance = creditAccountDetail.minMonthlyPayment;

        }
        else{
            //console.log("apr",creditAccountDetail.apr)
            totalOutstandingBalance = creditAccountDetail.totalBalanceDue;
            creditDebitMatchObj["apr"] = creditAccountDetail.apr;
            creditDebitMatchObj["minimumDue"] = creditAccountDetail.minBalanceDue;
        }
...
let getTotalBalanceCredit = (jsCredit) => {
    try {
        let totalCreditDue = jsCredit.banks.reduce((acc, val) => {
            let due = (val.accounts[0].accountType == "M")?val.accounts[0].minMonthlyPayment:val.accounts[0].totalBalanceDue;
            return acc + due;
        }, 0)
...
let filterData = (jsCredit,jsDebit) => {
    jsDebit.banks = jsDebit.banks.filter((debitAcc) => {
        return (debitAcc.accounts[0].availableBalance > 0)?true:false;
    })
    jsCredit.banks = jsCredit.banks.filter((creditAcc) => {
        if(!creditAcc.accounts[0].accountType.localeCompare("M"))
        {
            console.log("minmonthlypayment",creditAcc.accounts[0].totalBalanceDue)
            return (creditAcc.accounts[0].minMonthlyPayment > 0)?true:false;
        }
        else{
            console.log("totalbalance",creditAcc.accounts[0].totalBalanceDue)
            return (creditAcc.accounts[0].totalBalanceDue > 0)?true:false;
        }
    })
}
